...
// Create HTTP server for OAuth redirects and App Links (port 8081)
const HTTP_PORT = process.env.HTTP_PORT || 8081;
const SERVER_HOST = process.env.SERVER_HOST || '45.155.102.148';

const httpServer = http.createServer(async (req, res) => {
    const url = new URL(req.url, `http://${req.headers.host}`);
    
    // Handle assetlinks.json for Android App Links verification
    if (url.pathname === '/.well-known/assetlinks.json') {
        try {
            const assetlinksPath = path.join(path.dirname(fileURLToPath(import.meta.url)), '.well-known', 'assetlinks.json');
            const assetlinksContent = await fs.readFile(assetlinksPath, 'utf-8');
            res.writeHead(200, {
                'Content-Type': 'application/json',
                'Access-Control-Allow-Origin': '*'
            });
            res.end(assetlinksContent);
            console.log('✅ Served assetlinks.json for App Links verification');
        } catch (error) {
    ...
    // Handle OAuth callback (client-side only - implicit flow)
    if (url.pathname === '/oauth/callback') {
        // With implicit flow, Google redirects with id_token in the URL fragment (#)
        // We need to extract it and pass it to the app
        // Since fragments aren't sent to server, we'll use a simple redirect page that extracts it client-side
        ...
        if (error) {
            // Redirect with error
            const redirectUrl = `http://${SERVER_HOST}:${HTTP_PORT}/oauth/callback?error=${encodeURIComponent(error)}`;
            res.writeHead(302, {
            ...
            res.end(`
                <html>
                    ...
                    <body>
                        <p>OAuth error occurred. Redirecting to app...</p>
            ...
            // Return a page that extracts the id_token from the fragment and redirects to app
            // This is needed because URL fragments aren't sent to the server
                ...
                <html>
                    ...
                    <body>
                        ...
                        <script>
                            ...
                            if (idToken) {
                                // Redirect to app with id_token in query (App Link will intercept)
                                const redirectUrl = 'http://${SERVER_HOST}:${HTTP_PORT}/oauth/callback?id_token=' + 
                                    encodeURIComponent(idToken) + 
                                    (state ? '&state=' + encodeURIComponent(state) : '');
                                window.location.href = redirectUrl;
                            } else if (error) {
                                // Redirect with error
                                const redirectUrl = 'http://${SERVER_HOST}:${HTTP_PORT}/oauth/callback?error=' + 
                                    encodeURIComponent(error);
                                window.location.href = redirectUrl;
                            } else {
                                console.error('No id_token or error in OAuth callback');
                            }
                        </script>
                    </body>
                </html>
            `);
            console.log(`✅ OAuth callback: Processing implicit flow redirect (extracting id_token from fragment)`);
        }
        return;
    }
...
httpServer.listen(HTTP_PORT, '0.0.0.0', () => {
    console.log(`✅ HTTP server running on http://0.0.0.0:${HTTP_PORT} (for OAuth redirects and App Links)`);
    console.log(`   OAuth callback URL: http://${SERVER_HOST}:${HTTP_PORT}/oauth/callback`);
    console.log(`   App Links verification: http://${SERVER_HOST}:${HTTP_PORT}/.well-known/assetlinks.json`);
    console.log(`   Add OAuth callback URL to Google Cloud Console as an authorized redirect URI`);
});
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      } else if (data.type === 'googleOAuthCode') {
        // Handle Google OAuth authorization code (for Android WebView App Links)
        // Exchange authorization code for ID token
        if (!data.code) {
            if (ws.readyState === ws.OPEN) {
                ws.send(msgpack.encode({ type: 'error', message: 'Authorization code is required' }));
            }
            return;
        }

        try {
            const GOOGLE_CLIENT_ID = '87088971876-k8nrldpm91ohr0gh9vep2sdu6m4sv50m.apps.googleusercontent.com';
            // NOTE: You need to set GOOGLE_CLIENT_SECRET as an environment variable or in a config file
            // For security, never commit the client secret to version control
            const GOOGLE_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET || '';
            
            if (!GOOGLE_CLIENT_SECRET) {
                throw new Error('Google OAuth client secret not configured. Please set GOOGLE_CLIENT_SECRET environment variable.');
            }

            console.log(`🔐 Exchanging Google OAuth authorization code for ID token`);
            ...
            const tokenData = await new Promise((resolve, reject) => {
                const postData = new URLSearchParams({
                    ...
                    redirect_uri: `http://${SERVER_HOST}:${HTTP_PORT}/oauth/callback`,
            ...
            // Now process the ID token the same way as regular Google OAuth
            const credential = tokenData.id_token;
